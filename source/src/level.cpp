				...
				while (pTileA) {
					...
					if (pAnimation != NULL) {
						while (pAnimation) {
							...
							pAnimation = pAnimation->NextSiblingElement("animtion");
						}
					}
				...
				while (pData) {
					...
					if (pTile != NULL) {
						...
						while (pTile) {
							...
							//Calculate the postion of the tile in the tileset
				...
				if (pObject != NULL) {
					while (pObject) {
						...
						if (pPolyline != NULL) {
							...
							//Now we have each of the pair. Loop through the list of pairs
							//and split them into Vector2s and then store them in our points vector
							for (int i = 0; i < pairs.size(); i++) {
								std::vector<std::string> ps;
								Utils::split(pairs.at(i), ps, ',');
								points.push_back(Vector2(std::stoi(ps.at(0)), stoi(ps.at(1))));
							}
						}
